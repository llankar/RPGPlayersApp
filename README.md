...
- `backend/models.py` – Pydantic models shared across endpoints.
- `backend/storage.py` – JSON persistence utilities (`data/state.json`).
- `frontend/` – Svelte + TypeScript client (`src/routes/player.svelte`, `src/routes/display.svelte`).
- `frontend/package.json` – Front‑end dependencies and scripts.
- `requirements.txt` – Python dependencies.

## Extending the app
The code is organized into small modules to keep it readable. Consider adding authentication, richer diagram editing, or note search by extending the corresponding files under `backend/` and `frontend/src/`.
